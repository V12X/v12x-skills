...
| **[v12x-coherence](skills/v12x-coherence)** | **Visual coherence** audit — no design system needed. Infers the ruler the app already uses (alignment, spacing, radius, type) and flags where it contradicts itself: 2-3px misalignments, spacing off its own scale. Measures real geometry in the browser. Doesn't judge taste — measures consistency. Guidance in Portuguese. |
| **[v12x-flow](skills/v12x-flow)** | **Flow & usability** scan that **operates** the app (doesn't opine on a screenshot): drives it task by task, instruments console/network/controls, and separates what's **broken** (fact) from measurable friction and from **judgment** (opinion labeled by heuristic). Finds the button that does nothing, the missing error state, the flow that never completes. Verdict by blockers, never a score. Guidance in Portuguese. |
